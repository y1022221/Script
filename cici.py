...
class CCD:
    ...
    def user_info(self):
        ...
        if return_data['code'] != 200:
            ...
            need_exp = return_data.get('data').get('nextMonthProtectLevelNeedGrowthExp', 0) - return_data.get(
                'data').get('growthExp', 0)
        ...
        try:
            ...
            if num_data.get('status') != 'success':
                ...
                for _ in range(draw_num):
                    do_draw_res = self.send_request(do_draw_url, headers = self.zq_headers, data = payload,
                                                    method = 'POST')
    ...
    def open_box(self):
        if box_flag == 0:
            ...
            return
        # 获取盲盒类型信息
        type_url = 'https://fission-events.ccbft.com/activity/dmspblindbox/index/224/Q3Np1Kmp'
    ...
    def process_opening(self, pot_id):
        # 获取用户当前可用次数
        num_url = 'https://fission-events.ccbft.com/Component/draw/getUserCCB/224/Q3Np1Kmp'
        num_data = self.send_request(num_url, headers = self.zq_headers)
        self.sleep()
        ...
        while surplus_num > (draw_num - box_draw):
            open_url = 'https://fission-events.ccbft.com/activity/dmspblindbox/draw/224/Q3Np1Kmp'
